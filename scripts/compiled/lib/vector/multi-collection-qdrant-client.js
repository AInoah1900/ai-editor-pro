...
exports.MultiCollectionQdrantClient = void 0;
 ...
 */
class MultiCollectionQdrantClient {
    constructor() {
        this.pointIdCounters = new Map();
        this.initializedCollections = new Set();
        this.client = new js_client_rest_1.QdrantClient(getQdrantConfig());
        console.log('🚀 多集合Qdrant客户端初始化完成');
    }
    /**
     * 根据领域代码生成集合名称
     */
    getCollectionName(domainCode) {
     ...
     * 生成有效的点 ID
     */
    generatePointId(collectionName) {
    ...
    cleanPayload(payload) {
        ...
        for (const [key, value] of Object.entries(payload)) {
            ...
            if (typeof value === 'string' ||
                ...
                const cleanArray = value.filter(item => typeof item === 'string' ||
                    typeof item === 'number' ||
                    typeof item === 'boolean');
                ...
                const cleanNested = this.cleanPayload(value);
     ...
     * 初始化单个集合
     */
    async initializeCollection(domainCode) {
     ...
     * 添加向量点到指定领域集合
     */
    async upsertPoint(domainCode, id, vector, payload) {
        try {
            // 确保目标集合存在
            await this.initializeCollection(domainCode);
            const collectionName = this.getCollectionName(domainCode);
                ...
                points: [
                    {
                        id: pointId,
                        vector: vector,
                        payload: Object.assign(Object.assign({}, cleanPayload), { original_id: id, domain_code: domainCode, collection_name: collectionName }),
                    },
                ],
            });
     ...
     * 在指定领域集合中搜索相似向量
     */
    async searchInDomain(domainCode, queryVector, limit = 5, filter) {
        try {
            await this.initializeCollection(domainCode);
            const collectionName = this.getCollectionName(domainCode);
            // 验证向量维度
            ...
            return searchResponse.map((result) => {
                var _a;
                return ({
                    id: ((_a = result.payload) === null || _a === void 0 ? void 0 : _a.original_id) || String(result.id),
                    score: result.score,
     ...
     */
    async searchInMultipleDomains(domainCodes, queryVector, limitPerDomain = 5, filter) {
        console.log(`🔍 多领域并行搜索: ${domainCodes.join(', ')}`);
        // 并行搜索所有指定领域
        const searchPromises = domainCodes.map(domainCode => this.searchInDomain(domainCode, queryVector, limitPerDomain, filter));
        try {
     ...
     * 智能搜索：自动包含通用集合
     */
    async smartSearch(primaryDomainCode, queryVector, limitPerDomain = 5, filter) {
        // 搜索主要领域和通用集合
     ...
     * 删除指定领域集合中的向量点
     */
    async deletePoint(domainCode, id) {
        try {
            const collectionName = this.getCollectionName(domainCode);
     ...
     * 获取指定领域集合的统计信息
     */
    async getCollectionInfo(domainCode) {
        try {
            await this.initializeCollection(domainCode);
            const collectionName = this.getCollectionName(domainCode);
    ...
    async getAllCollectionsInfo() {
        ...
        const promises = JOURNAL_DOMAIN_CODES.map(domainCode => this.getCollectionInfo(domainCode));
        try {
...
exports.MultiCollectionQdrantClient = MultiCollectionQdrantClient;
