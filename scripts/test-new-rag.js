...
const { NewKnowledgeRetriever, DomainClassifier } = require('../lib/rag/new-knowledge-retriever');
...
async function testNewRAGSystem() {
  ...
  try {
    ...
    relevantKnowledge.forEach((item, index) => {
      console.log(`  ${index + 1}. ${item.content} (相关度: ${(item.relevance_score * 100).toFixed(1)}%)`);
    });
...
module.exports = { testNewRAGSystem }; 