    ...
    const dualClient = getDualDeepSeekClient(); // 使用现有实例，保持配置中心设置
        ...
        const apiPromise = dualClient.createChatCompletion({
          messages: [
            {
              role: 'system',
              content: `你是世界顶级的学术期刊编辑专家，专注于${domainInfo.domain}领域的稿件编辑加工，具备强大的语言理解能力与格式规范识别能力。你的任务是按照期刊出版标准对稿件进行检查、修改、润饰、标注和整理，确保符合学术规范和出版要求。
...
**学术写作风格规范**：
- 遵循${domainInfo.domain}领域的特定写作惯例
- 确保引用格式符合期刊要求（GB/T 7714或特定格式）
- 规范英文缩写的首次出现形式
- 统一时态和语态的使用

【四、学术规范与风险识别】
- 识别潜在的学术不端问题（引用不明、逻辑矛盾等）
- 检查数据与图表内容一致性
- 排查法律、事实或价值观风险点
- 确保学术表达的严谨性和准确性

**输出要求**：
请严格按照以下JSON格式返回校对结果，输出必须是有效的JSON字符串：
  ...
  "errors": [
    {
      ...
      "original": "具体错误文字",
      "suggestion": "专业修改建议",
      "reason": "基于编辑规范和知识库的详细修改原因",
      "category": "错误类别",
      "risks": "风险评估（如：无明显法律、事实或价值观问题）"
    }
  ]
}

**错误类型定义**：
- error: 确定错误（语法、术语、格式、标点等明确错误）
- warning: 疑似问题（表达不规范、逻辑不清、引用不当等）
- suggestion: 优化建议（表达改进、风格提升、学术严谨性增强）

**专业知识库资源**：
${formatKnowledge(multiKnowledgeResult.combined_knowledge)}

请基于上述专业知识库和编辑规范，对提交的稿件进行全面的编辑加工分析。`
            },
            {
              role: 'user',
...
function buildEnhancedPromptWithMultiKnowledge(
  ...
  const basePrompt = `请对以下${domainInfo.domain}领域学术稿件进行专业的编辑加工分析，按照期刊出版标准进行全面校对，并严格按照JSON格式返回结果。

**可用专业知识库资源**：

【专属知识库】(${multiKnowledgeResult.private_knowledge.length}条专业知识)：
${formatKnowledgeWithSource(multiKnowledgeResult.private_knowledge, '专属知识库')}

【共享知识库】(${multiKnowledgeResult.shared_knowledge.length}条通用知识)：
...
📐 **格式规范层面**：
- 标点符号规范性（GB/T 15834-2011标准）
- 数字、单位、符号使用标准化
- 图表编号、参考文献格式检查
- ${domainInfo.domain}领域特定格式要求
...
**输出格式要求**：
请严格按照以下JSON格式返回编辑加工结果，确保输出是有效的JSON字符串：
...
请基于上述专业知识库和编辑规范，对稿件进行全面的编辑加工分析，严格按照JSON格式返回结果。`;
...
function generateRiskAssessmentForError(errorType: string, category: string, original: string): string {
  ...
  const riskAssessments: Record<string, string> = {
    // 格式规范类
    '格式规范': '无明显法律、事实或价值观问题，属于格式规范性问题。',
    '标点符号': '无明显法律、事实或价值观问题，属于格式规范性问题。',
    '格式错误': '无明显法律、事实或价值观问题，属于编辑规范性问题。',
    
    // 语言表达类
    '语言表达': '无明显法律、事实或价值观问题，属于语言表达优化。',
    '词汇使用': '无明显法律、事实或价值观问题，属于语言表达优化。',
    
    // 学术规范类
    '术语与风格': '无明显法律风险，需确保专业术语准确性避免学术误解。',
    '术语使用': '无明显法律风险，需确保专业术语准确性避免学术误解。',
    '文档结构': '无法律风险，需注意学术规范性和逻辑完整性。',
    '内容完整性': '无法律风险，属于学术写作质量提升建议。',
    '学术规范': '无法律风险，需注意学术规范性和逻辑完整性。',
    
    // 其他类别
    '其他问题': '经风险评估，无明显法律、事实或价值观问题。'
  };
  
  return riskAssessments[category] || '经风险评估，无明显法律、事实或价值观问题。';
}
