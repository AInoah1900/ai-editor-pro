...
import { MultiCollectionQdrantClient } from '../vector/multi-collection-qdrant-client';
import { getDualDeepSeekClient } from '../deepseek/deepseek-dual-client';

/**
 * 多集合知识项接口 (简化版)
 */
interface MultiCollectionKnowledgeItem {
  id?: string;
 ...
 */
export class MultiCollectionDatabaseModels extends DatabaseModels {
  private multiCollectionQdrant: MultiCollectionQdrantClient;

  constructor() {
    super();
    this.multiCollectionQdrant = new MultiCollectionQdrantClient();
   ...
   * 初始化多集合知识库架构
   */
  async initializeMultiCollectionArchitecture(): Promise<{
    success: boolean;
    message: string;
    collections_result?: any;
    error?: string;
  }> {
    try {
   ...
   */
  private async intelligentDomainDetection(content: string, context?: string): Promise<string> {
    try {
   ...
   * 添加知识项到多集合架构
   */
  async addKnowledgeItemToMultiCollection(knowledgeItem: MultiCollectionKnowledgeItem): Promise<{
    success: boolean;
    message: string;
    knowledge_id?: string;
    ...
    try {
      ...
      const qdrantVectorId = `knowledge_${knowledgeId}_${Date.now()}`;
              ...
              await this.multiCollectionQdrant.upsertPoint(
          targetDomain,
          qdrantVectorId,
          vector,
          {
  ...
  async intelligentSearch(
    ...
    try {
      ...
      // 3. 执行多领域并行搜索
      const vectorResults = await this.multiCollectionQdrant.searchInMultipleDomains(
        searchDomains,
        queryVector,
        limit
      );
      ...
      const results = vectorResults.map(result => ({
        id: result.payload.knowledge_id as string || result.id,
        score: result.score,
   ...
   * 获取多集合架构统计信息
   */
  async getMultiCollectionStats(): Promise<{
    success: boolean;
    stats: {
   ...
   * 数据迁移：从单集合迁移到多集合架构
   */
  async migrateToMultiCollection(): Promise<{
    success: boolean;
    message: string;
   ...
   */
  getMultiCollectionQdrantClient(): MultiCollectionQdrantClient {
